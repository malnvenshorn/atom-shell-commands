( function( window, atom, require, module, undefined ) {
	...
	function storeProject() {
    	...
    	const file = buffer ? buffer : (editor ? editor.file : null);
    	const project = file && file.getParent() ? file.getParent().path : null;

    	if (project !== commands.currentProject) {
      		commands.currentProject = project ? project : null;
      		return true;
		} else {
      		return false;
		}
	...
	function registerEvents() {
   		const subscriptions = commands.subscriptions;
	...
	function resolveConfigPath(
		configFilePath = atom.config.get('atom-shell-commands.configurationFilePath'),
	    ...
	    if (configPath) {
	    	commands.configFilePath = configPath;
	        return true;
	...
	function resolveProjectConfigPath(
		configFilePath = atom.config.get('atom-shell-commands.projectConfigurationFilePath')) {
		const projectRoot = atom.project.getPaths()[0];

		if (projectRoot) {
			let configPath = path.resolve(projectRoot, configFilePath);
		    ...
		    if (configPath) {
		    	commands.projectConfigFilePath = configPath;
		        return true;
			}
		}
	...
	function loadConfig() {
		...
		if (commands.configFilePath) {
			config = cson.requireCSONFile(commands.configFilePath);
		}

		if (commands.projectConfigFilePath) {
			const projConfig = cson.requireCSONFile(commands.projectConfigFilePath);
	...
	function reloadCommands() {
		...
		for (let disposable of commands.registeredAtomCommands) {
		...
		commands.registeredAtomCommands = [];
		...
		for(let command of shellCommands) {
			...
			commands.registeredAtomCommands.push( atomCommand );
			commands.registeredAtomCommands.push( menuEntry );
			...
			if (keymap) {
				...
				commands.registeredAtomCommands.push(entry);
