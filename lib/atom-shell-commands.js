( function( window, atom, require, module, undefined ) {
	...
	self.projectConfigFilePath = null;
	self.registeredAtomCommands = [];
	...
	function resolveProjectConfigPath(
		...
		if (projectRoot) {
		    ...
		    if (configPath) {
		    	self.projectConfigFilePath = configPath;
		        return true;
			}
		}
	...
	function loadConfig() {
		...
		if (self.configFilePath) {
			config = cson.requireCSONFile(self.configFilePath);
		}

		if (self.projectConfigFilePath) {
			const projConfig = cson.requireCSONFile(self.projectConfigFilePath);
	...
	function reloadCommands() {
		...
		for (disposable of self.registeredAtomCommands) {
		...
		for(let command of shellCommands) {
			...
			self.registeredAtomCommands.push( atomCommand );
			self.registeredAtomCommands.push( menuEntry );
			...
			if (keymap) {
				...
				self.registeredAtomCommands.push(entry);
